...
: vocab-match ( addr n vptr -- addr n [vptr' | 0 ] [typ_val | -1] )
    ...
    begin
        dup 1+ swap c@
        ( f s0 n0'  R: addr n tv )
        dup $80 and -rot $7f and
        ( f last? s0 n0  R: addr n tv )
        2dup + -rot dup 2r@ rot over = if
            ( f last? p1 s0 n0 addr n  R: addr n tv )
...
: english ( -- true | false )
    ...
    user-input cleave analyze 0= if
        ( addr n )
        2drop false exit
    then
    ( addr n tv )
